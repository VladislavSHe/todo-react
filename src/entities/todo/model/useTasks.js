...
const useTasks = () => {
  ...
  const deleteAllTasks = useCallback(() => {
    const isConfirm = confirm('Are you sure that you want to delete all tasks?')
  ...
  const addTask = useCallback((title, callbackAfterAdding) => {
    ...
    tasksAPI.add(newTask)
      .then((addedTask) => {
        ...
        callbackAfterAdding()
        setSearchQuery('') // Чтобы увидеть новую задачу, поиск отключается
        newTaskInputRef.current.focus() // Фокус на поле ввода задачи
